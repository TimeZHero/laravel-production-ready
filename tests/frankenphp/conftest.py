"""
Test fixtures for FrankenPHP Docker image integration tests.

Scaffolds a Laravel app, builds Docker images for frankenphp x TARGET
...
IMAGE_PREFIX = "laravel-frankenphp-test"
...
def _docker_build(path: str, tag: str, target: str, engine: str):
    ...
    cmd = [
        "docker", "build",
        "-f", os.path.join(path, "Dockerfile.frankenphp"),
        "--tag", tag,
        "--target", target,
...
def laravel_app():
    ...
    FrankenPHP. Returns the path to the build context.
    """
    tmpdir = tempfile.mkdtemp(prefix="laravel-frankenphp-test-")
    ...
    try:
        ...
        for item in ("Dockerfile.frankenphp", "confs", "scripts"):
        ...
        # 3. Install octane (no roadrunner packages needed for frankenphp)
        _composer(
            tmpdir,
        ...
        # 6. Install npm dependencies (chokidar needed for octane --watch in dev)
        subprocess.run(
        ...
        # 7. Patch start-container.sh: skip migrations (no real DB in tests)
...
def docker_images(docker_client, laravel_app):
    """Build FrankenPHP Docker images (ENGINE x TARGET)."""
    built_tags = []
...
class ContainerInfo:
    ...
    @property
    def is_frankenphp(self) -> bool:
        return self.engine == "frankenphp"
    ...
    def php_ini_get(self, directive: str) -> str:
        """Get a PHP ini value via a temp script (avoids shell quoting issues)."""
        return self.exec(
            f"sh -c 'echo \"<?php echo ini_get(\\\"{directive}\\\");\" > /tmp/_q.php && php /tmp/_q.php'"
        ).strip()

    def php_modules(self) -> list[str]:
        """Get loaded PHP modules via a temp script (avoids shell quoting issues)."""
        output = self.exec(
            "sh -c 'echo \"<?php foreach(get_loaded_extensions() as \\$e) echo strtolower(\\$e).PHP_EOL;\" > /tmp/_q.php && php /tmp/_q.php'"
        )
        return [m.strip() for m in output.splitlines() if m.strip()]
...
def all_containers(docker_client, docker_images, laravel_app):
    ...
    for engine, target in COMBOS:
        name = f"laravel-frankenphp-test-{engine}-{target}"
        try:
    ...
    for engine, target in COMBOS:
        ...
        run_kwargs = dict(
            ...
            name=f"laravel-frankenphp-test-{combo}",
            remove=False,
        )
